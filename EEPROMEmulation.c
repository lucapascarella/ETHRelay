...
unsigned int DataEERead(unsigned int addr) {
    ...
    // Find the active page.
    for (currentPage = 0; (currentPage < NUM_DATA_EE_PAGES) && (GetPageStatus(bank, currentPage, STATUS_CURRENT) == PAGE_NOT_CURRENT); currentPage++) {
    }
...
//void FlashWrite(far unsigned char * source_addr, unsigned char length, far unsigned char * dest_addr) {
//
