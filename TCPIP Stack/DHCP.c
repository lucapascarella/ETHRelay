...
void DHCPTempIPAddr(void)
{
	appConfig.ip.fields.MyIPAddr = DHCPClient.tempIPAddress;  
}
				...
				switch(_DHCPReceive())
				{
					case DHCP_ACK_MESSAGE:
						...
						if(DHCPClient.validValues.bits.Mask)
							appConfig.ip.fields.MyMask = DHCPClient.tempMask;
						if(DHCPClient.validValues.bits.Gateway)
							appConfig.ip.fields.MyGateway = DHCPClient.tempGateway;
						#if defined(STACK_USE_DNS)
							if(DHCPClient.validValues.bits.DNS)
								appConfig.ip.fields.PrimaryDNSServer.Val = DHCPClient.tempDNS.Val;
							appConfig.ip.fields.SecondaryDNSServer.Val = 0x00000000ul;
							if(DHCPClient.validValues.bits.DNS2)
								appConfig.ip.fields.SecondaryDNSServer.Val = DHCPClient.tempDNS2.Val;
						#endif
...
static BYTE _DHCPReceive(void)
{
	...
	********************************************************************/
	...
	if ( v == BOOT_REPLY )
	{
		...
		{
			UDPGet(&v);
			if ( v != appConfig.ip.fields.MyMACAddr.v[i])
				goto UDPInvalid;
		}
...
static void _DHCPSend(BYTE messageType, BOOL bRenewing)
{
	...
	UDPPutArray((BYTE*)&appConfig.ip.fields.MyMACAddr, sizeof(appConfig.ip.fields.MyMACAddr));
	...
	MyIP.Val = appConfig.ip.fields.MyIPAddr.Val;
	if(!bRenewing)
		appConfig.ip.fields.MyIPAddr.Val = 0x00000000;
	UDPFlush();
	appConfig.ip.fields.MyIPAddr.Val = MyIP.Val;
