...
//    packet->SenderMACAddr = AppConfig.MyMACAddr;	// HI-TECH PICC-18 compiler can't handle this statement, use memcpy() as a workaround
	memcpy(&packet->SenderMACAddr, (void*)&appConfig.ip.fields.MyMACAddr, sizeof(packet->SenderMACAddr));
#ifdef STACK_USE_ZEROCONF_LINK_LOCAL
    //packet->SenderIPAddr  = AppConfig.MyIPAddr; /* Removed for ZCLL, SenderIPAddr should be filled in */
#else
    packet->SenderIPAddr  = appConfig.ip.fields.MyIPAddr;
#endif
...
/*                #if defined(STACK_USE_AUTO_IP)
			...
			{
				if(packet.TargetIPAddr.Val != appConfig.ip.fields.MyIPAddr.Val)
				{
					return TRUE;
				}
#ifdef STACK_USE_ZEROCONF_LINK_LOCAL
...
#define KS_ARP_IP_MULTICAST_HACK y
	...
	packet.TargetIPAddr			= ((appConfig.ip.fields.MyIPAddr.Val ^ IPAddr->Val) & appConfig.ip.fields.MyMask.Val) ? appConfig.ip.fields.MyGateway : *IPAddr;
#ifdef STACK_USE_ZEROCONF_LINK_LOCAL
	packet.SenderIPAddr			= appConfig.ip.fields.MyIPAddr;
#endif
...
BOOL ARPIsResolved(IP_ADDR* IPAddr, MAC_ADDR* MACAddr)
{
    if((Cache.IPAddr.Val == IPAddr->Val) || 
	  ((Cache.IPAddr.Val == appConfig.ip.fields.MyGateway.Val) && ((appConfig.ip.fields.MyIPAddr.Val ^ IPAddr->Val) & appConfig.ip.fields.MyMask.Val)))
    {
