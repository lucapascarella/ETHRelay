...
BOOL DNSIsResolved(IP_ADDR* HostIP)
{
	...
	switch(smDNS)
	{
		...
		case DNS_ARP_START_RESOLVE:
			ARPResolve(&appConfig.ip.fields.PrimaryDNSServer);
			vARPAttemptCount++;
			StartTime = TickGet();
		...
		case DNS_ARP_RESOLVE:
			if(!ARPIsResolved(&appConfig.ip.fields.PrimaryDNSServer, &ResolvedInfo.MACAddr))
			{
				if(TickGet() - StartTime > DNS_TIMEOUT)
					smDNS = (vARPAttemptCount >= 3u) ? DNS_FAIL : DNS_ARP_START_RESOLVE;
				break;
			}
			ResolvedInfo.IPAddr.Val = appConfig.ip.fields.PrimaryDNSServer.Val;
...
DoneSearchingRecords:
		...
		case DNS_FAIL:
			...
			if(appConfig.ip.fields.SecondaryDNSServer.Val)
			{
				appConfig.ip.fields.PrimaryDNSServer.Val ^= appConfig.ip.fields.SecondaryDNSServer.Val;
				appConfig.ip.fields.SecondaryDNSServer.Val ^= appConfig.ip.fields.PrimaryDNSServer.Val;
				appConfig.ip.fields.PrimaryDNSServer.Val ^= appConfig.ip.fields.SecondaryDNSServer.Val;
