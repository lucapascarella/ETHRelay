...
void AnnounceIP(void)
{
	...
	//MySocket = UDPOpenEx((DWORD) (PTR_BASE) "192.168.3.10", UDP_OPEN_RAM_HOST,2860, ANNOUNCE_PORT);
	...
	UDPPutArray((BYTE*)appConfig.ip.fields.NetBIOSName, sizeof(appConfig.ip.fields.NetBIOSName)-1);
	UDPPut('\r');
	UDPPut('\n');
	...
	while(1)
	{
		UDPPut(btohexa_high(appConfig.ip.fields.MyMACAddr.v[i]));
	    UDPPut(btohexa_low(appConfig.ip.fields.MyMACAddr.v[i]));
	    if(++i == 6u)
	    	break;
	    UDPPut('-');
	}
