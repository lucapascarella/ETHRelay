...
void NBNSTask(void)
{
	...
	switch(smNBNS)
	{
		...
		case NBNS_LISTEN:
        	...
        	if((remoteNode.IPAddr.Val & appConfig.ip.fields.MyMask.Val) != (appConfig.ip.fields.MyIPAddr.Val & appConfig.ip.fields.MyMask.Val))
			{
				UDPDiscard();
				break;
			}
			...
			while(NBNSHeader.Questions.Val--)
			{
				...
				if(Type.Val == 0x0020u && Class.Val == 0x0001u && memcmp((void*)NameString, (void*)appConfig.ip.fields.NetBIOSName, sizeof(appConfig.ip.fields.NetBIOSName)) == 0)
				{
					...
					NBNSPutName(appConfig.ip.fields.NetBIOSName);
					...
					UDPPut(0x00);
					UDPPut(appConfig.ip.fields.MyIPAddr.v[0]);	// Put out IP address
					UDPPut(appConfig.ip.fields.MyIPAddr.v[1]);
					UDPPut(appConfig.ip.fields.MyIPAddr.v[2]);
					UDPPut(appConfig.ip.fields.MyIPAddr.v[3]);
