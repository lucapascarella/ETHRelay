...
#include "TCPIP Stack/TCPIP.h"
        ...
        #include "TCPIPStack/WFConsole.h"
    #endif
    #if defined( STACK_USE_EZ_CONFIG ) || defined( EZ_CONFIG_SCAN )
        #include "TCPIPStack/WFEasyConfig.h"
    #endif
	#include "TCPIPStack/WFApi.h"
...
void StackInit(void)
{
	...
	DHCPInit(0);
    if(!appConfig.ip.fields.flags.bits.bIsDHCPEnabled)
    {
        DHCPDisable(0);
    }
#endif
...
void StackTask(void)
{
	...
	if(appConfig.ip.fields.flags.bits.bIsDHCPEnabled)
	{
		...
		if(bCurrentLinkState != bLastLinkState)
		{
			...
			{
				appConfig.ip.fields.MyIPAddr.Val = appConfig.ip.fields.DefaultIPAddr.Val;
				appConfig.ip.fields.MyMask.Val = appConfig.ip.fields.DefaultMask.Val;
				appConfig.ip.fields.flags.bits.bInConfigMode = TRUE;
				DHCPInit(0);
			}
		}
		...
		if(DHCPIsBound(0))
			appConfig.ip.fields.flags.bits.bInConfigMode = FALSE;
	}
	#endif
		...
		switch(cFrameType)
		{
			...
			case MAC_IP:
				...
				if(cIPFrameType == IP_PROT_ICMP)
				{
					...
					if( (tempLocalIP.Val == appConfig.ip.fields.MyIPAddr.Val) ||
...
#endif
						(tempLocalIP.Val == ((appConfig.ip.fields.MyIPAddr.Val & appConfig.ip.fields.MyMask.Val) | ~appConfig.ip.fields.MyMask.Val)))
					{
